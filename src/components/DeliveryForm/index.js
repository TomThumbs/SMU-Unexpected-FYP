...
class DeliveryFormBase extends Component {
  ...
  componentDidMount() { 
    ...
    this.props.firebase.fs.collection('Catering_orders').doc("Event 2").get().then(doc=> {
          // console.log('Found subcollection with id:', doc.data().Customer.id);
          this.setState({
            name: doc.data().Customer.id
          })
          this.props.firebase.fs.collection('Customers').doc(this.state.name).get().then(docu=> {
            // console.log('Found subcollection with id:', docu.data().HP);
            this.setState({
              contact: docu.data().HP, //Wilson, when you need to use the contact var, use this "this.state.contact"
              name: docu.data().Name
            })            
        });            
      });    
