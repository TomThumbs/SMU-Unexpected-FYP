...
class DeliveryFormBase extends Component {
  ...
  componentDidMount() { //problem cannot get data from delivery selection
    // this.props.firebase.fs.collection('Catering_orders').doc(this.props.location.orderid).get().then(doc=> {
    this.props.firebase.fs.collection('Catering_orders').doc("Event 2").get().then(doc=> {
          this.setState({
            ...
            pax: doc.data().Pax,
          //   name: doc.data().Customer, //reference is impossible but collection is possible. consider it. 
          //problem getting cust ref. 
          }) 
          console.log(doc.id)
      })
  }
  ...
  renderSubmit() {
    if (this.state.cleanReady == true && (this.state.allItems == true && this.state.foodWrap == true)) {
      // console.log("clear")
  ...
  render() {
    return (  
<div>
      <h1>{this.props.location.orderid}</h1>
      <React.Fragment>
