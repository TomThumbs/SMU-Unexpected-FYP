			...
			this.props.firebase.fs
				...
				.onSnapshot((changes) => {
					...
					changes.forEach((change) => {
						...
						if (
							this.state.counter <= Object.keys(this.state.heatersUsed).length
						) {
							...
							if (change.data().name === "temperature sensor") {
								...
								//set to dict
								Object.assign(this.state.thresholds, {
									[dishname]: change.data().minimum,
								});
								this.setState({
								...
								.onSnapshot((snapshot) => {
									...
									changes.forEach((change) => {
										...
										if (change.doc.data().ID === dish && counte === 0) {
											counte = 1;
											Object.assign(this.state.temps, {
												[dishname]: change.doc.data().temp,
											});
											this.setState({
