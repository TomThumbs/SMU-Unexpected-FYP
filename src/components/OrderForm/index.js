...
class OrderFormBase extends Component {
  ...
  componentDidMount() {  
    ...
    this.props.firebase.fs.collection('Menu').get().then(snapshot => {snapshot.forEach(doc => {
      this.state.menu_dict[doc.id] = []
  ...
  renderSubmit() {
    if (this.state.date.length !== 0 &&
      ...
      return <h1>Please ensure that all fields are filled.</h1>
    }
  }
