...
classes = useStyles
  ...
  onSubmit = event => {
    ...
    let menuAcc = ''
    if (this.state.bva=="true") {
        menuAcc = this.state.menu_dict["bva"] 
    } 
    if (this.state.bvb=="true") {
      if (menuAcc.length !== 0) {
        menuAcc = menuAcc +", "+ this.state.menu_dict["bvb"] 
      } else {
        menuAcc = this.state.menu_dict["bvb"] 
      }
    }
    if (this.state.rca=="true") {
      if (menuAcc.length !== 0) {
        menuAcc = menuAcc +", "+ this.state.menu_dict["rca"] 
      } else {
        menuAcc = this.state.menu_dict["rca"] 
      }
    }
    if (this.state.rcb=="true") {
      if (menuAcc.length !== 0) {
        menuAcc = menuAcc +", "+ this.state.menu_dict["rcb"] 
      } else {
        menuAcc = this.state.menu_dict["rcb"] 
      }
    }
    if (this.state.dea=="true") {
      if (menuAcc.length !== 0) {
        menuAcc = menuAcc +", "+ this.state.menu_dict["dea"] 
      } else {
        menuAcc = this.state.menu_dict["dea"] 
      }
    }
    if (this.state.deb=="true") {
      if (menuAcc.length !== 0) {
        menuAcc = menuAcc +", "+ this.state.menu_dict["deb"] 
      } else {
        menuAcc = this.state.menu_dict["deb"] 
      }
    }
    if (this.state.fia=="true") {
      if (menuAcc.length !== 0) {
        menuAcc = menuAcc +", "+ this.state.menu_dict["fia"] 
      } else {
        menuAcc = this.state.menu_dict["fia"] 
      }
    }
    if (this.state.fib=="true") {
      if (menuAcc.length !== 0) {
        menuAcc = menuAcc +", "+ this.state.menu_dict["fib"] 
      } else {
        menuAcc = this.state.menu_dict["fib"] 
      }
    }
  ...
  handleChange= name => event =>  {
    ...
    if ((name === "rca") && this.state.rca === "true") {
      this.setState({...this.props, [name]: false});
    }
    if ((name === "rcb") && this.state.rcb === "true") {
      this.setState({...this.props, [name]: false});
    }
    if ((name === "dea") && this.state.dea === "true") {
      this.setState({...this.props, [name]: false});
    }
    if ((name === "deb") && this.state.deb === "true") {
      this.setState({...this.props, [name]: false});
    }
    if ((name === "fia") && this.state.fia === "true") {
      this.setState({...this.props, [name]: false});
    }
    if ((name === "fib") && this.state.fib === "true") {
      this.setState({...this.props, [name]: false});
    }
    if ((name === "bva") && this.state.bva === "true") {
      this.setState({...this.props, [name]: false});
    }
    if ((name === "bvb") && this.state.bvb === "true") {
      this.setState({...this.props, [name]: false});
    }
